...
export const ProductFilters: React.FC<ProductFiltersProps> = ({ onFiltersChange: _onFiltersChange }) => {
  const [priceRange, setPriceRange] = React.useState([0, 2000]);
  const [selectedCollections, setSelectedCollections] = React.useState<string[]>([]);
  const [selectedModels, setSelectedModels] = React.useState<string[]>([]);

  const collections = ['SPORT', 'LIFESTYLE', 'PRISMATIC'];

  const models = ['Music Shield', 'Falcon', 'Shield', 'Prime', 'Duck Classic', 'Dragon', 'Aura', 'Euphoria'];

  const handleCollectionChange = (collection: string, checked: boolean): void => {
    if (checked) {
  ...
  const handleModelChange = (model: string, checked: boolean): void => {
    if (checked) {
