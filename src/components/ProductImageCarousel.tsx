        ...
        <Card className="overflow-hidden group">
          <CardContent className="p-0 relative">
            ...
            >
              ...
              {/* Image type badge */}
              {currentImage.type && (
                <Badge className="absolute top-4 left-4 bg-black/70 text-white">{currentImage.type}</Badge>
              )}
            ...
            {images.map((_, index) => (
              <button
                ...
                className={cn(
                  ...
                  selectedIndex === index ? 'bg-primary scale-125' : 'bg-muted hover:bg-muted-foreground/50',
                )}
                onClick={() => setSelectedIndex(index)}
              />
            ))}
        ...
        <div className="fixed inset-0 z-50 bg-black flex items-center justify-center">
          <div className="relative w-full h-full flex items-center justify-center p-4">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/70 text-white px-4 py-2 rounded-full">
              ...
              {images.length > 1 && (
                <Button variant="ghost" size="icon" className="text-white hover:bg-white/20" onClick={toggleAutoPlay}>
                  {isAutoPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
                </Button>
              )}
            </div>
